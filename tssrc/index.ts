...
class Discord {
    ...
    async record(url: string, image: ArrayBuffer, message: Message) {
        ...
        if (RECORD_G) {
            ...
            await fs.mkdir(`${IMAGEDIR}${id}/`, function (err: Error) {
                if (err) throw new Error(`FILE_CREATION_FAILED: ${err}`)
            }) // create new dir with id
        } else { if (!fs.existsSync('./temp')) fs.mkdirSync('./temp') }
        var obj = {
            date: Date.now(),
        ...
        if (RECORD_G) fs.writeFileSync(`${IMAGEDIR}${id}/results.json`, JSON.stringify(obj, null, 2))
    }
    async image(data: any, fileID: string): Promise<any> {
        var self = this, p: string = (RECORD_G ? `${fileID}/outImage.png` : `./temp/outImage${makeid(16)}.png`)
            ...
            img.metadata()
                .then((imgdata: any) => {
                    ...
                    img.extract({ left: kdi.l, top: kdi.t, width: kdi.w, height: kdi.h })
                        ...
                        .normalise()
                        .png()
                        .toFile(p, function (err: Error, info: any) {
                            if (err) {
                                self.track?.update('error', false)
                                debug(err)
                                reject(err)
        ...
        axios.get('https://api.github.com/repos/B1ANK3/Discord_100KC/releases/latest', { responseType: 'JSON' })
            .then((res: AxiosResponse) => {
                if (res.status == 200) {
                    ...
                    } else { console.log(`Version is up to date.`) }
                } else console.log(`Unable to get latest version. Code: ${res.status}`)
            }).catch((err: AxiosError) => { console.log(`Unable to get latest version. ${err.code || ''}`) })
    }
